...
@section('content')
    <form action="/reportevisitas/{{$visita->id}}" method="POST">
        ...
        <div class="mb-3">
          ...
          <input type="datetime-local" class="form-control" name="fhingreso" id="fhingreso" tabindex="2" value="{{$visita->fh_ingreso}}">
        </div>
        <div class="mb-3">
          <label for="fhsalida" class="form-label">Fecha Salida</label>
          <input type="datetime-local" class="form-control" name="fhsalida" id="fhsalida" tabindex="3" value="{{$visita->fh_salida}}">
        </div>
